...
function safeValue(
  ...
  if (!isFinite(value)) {
    if (__DEV__) console.warn(`[BLEValidator] Non-finite value for "${fieldName}": ${value}`);
    return undefined;
  }
...
export function validateTreadmillData(raw: TreadmillData): TreadmillData {
  return {
    instantaneousSpeed:   safeValue(raw.instantaneousSpeed,  VALID_RANGES.speed_kmh,       'instantaneousSpeed') ?? 0,
    ...
    positiveElevationGain:safeValue(raw.positiveElevationGain,{min:0,max:5000},             'positiveElevationGain'),
    negativeElevationGain:safeValue(raw.negativeElevationGain,{min:0,max:5000},             'negativeElevationGain'),
    instantaneousHeartRate:safeValue(raw.instantaneousHeartRate, VALID_RANGES.heart_rate_bpm,'instantaneousHeartRate'),
    averageHeartRate:     safeValue(raw.averageHeartRate,     VALID_RANGES.heart_rate_bpm,  'averageHeartRate'),
    metabolicEquivalent:  safeValue(raw.metabolicEquivalent,  VALID_RANGES.met,             'metabolicEquivalent'),
...
export function validateIndoorBikeData(raw: IndoorBikeData): IndoorBikeData {
  return {
    instantaneousSpeed:   safeValue(raw.instantaneousSpeed,   VALID_RANGES.speed_kmh,      'instantaneousSpeed') ?? 0,
    averageSpeed:         safeValue(raw.averageSpeed,         VALID_RANGES.speed_kmh,      'averageSpeed'),
    instantaneousCadence: safeValue(raw.instantaneousCadence, VALID_RANGES.cadence_rpm,    'instantaneousCadence'),
    ...
    instantaneousPower:   safeValue(raw.instantaneousPower,   VALID_RANGES.power_watts,    'instantaneousPower'),
...
export function validateRowerData(raw: RowerData): RowerData {
  return {
    ...
    averageStrokeRate:    safeValue(raw.averageStrokeRate,    VALID_RANGES.stroke_rate,    'averageStrokeRate'),
    totalDistance:        safeValue(raw.totalDistance,        VALID_RANGES.distance_m,     'totalDistance'),
    instantaneousPace:    safeValue(raw.instantaneousPace,    VALID_RANGES.pace_s500m,     'instantaneousPace'),
...
export function validateHeartRateData(raw: HeartRateData): HeartRateData | null {
  ...
  if (bpm === undefined) {
    if (__DEV__) console.warn('[BLEValidator] HR packet has invalid bpm, discarding.');
    return null;
  }
  return {
    bpm,
    ...
    rrIntervals: raw.rrIntervals
      ?.slice(0, 8)
      .filter(rr => isFinite(rr) && rr > 200 && rr < 3000),
  };
}
...
export function validateConcept2Data(raw: Concept2RowingData): Concept2RowingData {
  return {
    ...
    totalWorkDistance:  safeValue(raw.totalWorkDistance,  VALID_RANGES.distance_m,    'totalWorkDistance') ?? 0,
    ...
    instantaneousPower: safeValue(raw.instantaneousPower, VALID_RANGES.power_watts,   'instantaneousPower') ?? 0,
    averagePower:       safeValue(raw.averagePower,       VALID_RANGES.power_watts,   'averagePower') ?? 0,
    averageCalories:    safeValue(raw.averageCalories,    VALID_RANGES.calories_kcal, 'averageCalories') ?? 0,
    heartRate:          safeValue(raw.heartRate,          {min:0, max:250},            'heartRate') ?? 0,
  };
}
...
export function isValidBLEPayload(base64: string | null | undefined): boolean {
  ...
  if (base64.length > 256) {
    if (__DEV__) console.warn('[BLEValidator] Payload too large, ignoring.');
    return false;
  }
